<?php
	...
	$COMPANY = array();
	if ($companyid) {
		$query = "SELECT * FROM companies WHERE id = '".res($companyid)."'; ";
		$result = qedb($query);
		$COMPANY = qrow($result);
	}
?>
...
<body class="sub-nav profile-body">
	...
	<form class="form-inline" method="GET" action="/company.php">

    <table class="table table-header">
		<tr>
			<td class="col-md-2">
<?php if ($VIEW_MODE AND $companyid) { ?>
	            <a href="/manage_company.php?companyid=<?= $companyid; ?>" class="btn btn-default icon" data-toggle="tooltip" title="Edit" data-placement="bottom"><i class="fa fa-pencil"></i></a>
...
<?php } else if ($companyid) { ?>
	            <a href="/company.php?companyid=<?= $companyid; ?>" class="btn btn-default" data-toggle="tooltip" title="Cancel Changes" data-placement="bottom"><i class="fa fa-close"></i></a>
<?php } ?>
			</td>
	...
	<form class="form-inline" method="POST" action="/save-company.php" enctype="multipart/form-data" name="profile_form">
...
?>
	...
	<?php } ?>
								<tr class="warning">
									<td><strong>DEFAULT RFQ EMAIL</strong></td>
									<td>
		<?php if ($VIEW_MODE) { ?>
										<?= $COMPANY['default_email']; ?>
		<?php } else { ?>
										<input type="text" name="default_email" value="<?= $COMPANY['default_email']; ?>" class="form-control input-sm">
		<?php } ?>
									</td>
									<td> </td>
									<td> </td>
