<?php
	...
	while ($r = mysqli_fetch_assoc($result)) {
		...
		$queryI = "SELECT * FROM inventory WHERE partid = $partid AND serial_no = '".res($serial_no)."'; ";
		$resultI = qdb($queryI) OR die(qe().'<BR>'.$queryI);
		if (mysqli_num_rows($resultI)==0) {
			die("Could not find our inventory record");
		}
		$I = mysqli_fetch_assoc($resultI);
		$inventoryid = $I['id'];

		if ($I['sales_item_id'] OR $I['repair_item_id']) {
			die("Found a sales/repair item id");
		}

		if (! $I['returns_item_id']) {
			die("No returns item id");
		} else if (! $I['purchase_item_id']) {
			die("No purchase item id");
		}

		$query2 = "SELECT po_number, price FROM purchase_items WHERE id = '".$I['purchase_item_id']."'; ";
		...
		$query3 .= "'".res($ship_to)."', 'RTV from PO# ".$po_number.", was RMA ".trim($rma.chr(10).$reason.chr(10).$freight_acct)."', 'Active'); ";
		$result3 = qdb($query3) OR die(qe().'<BR>'.$query3);
echo $query3.'<BR>';
		$so_number = 999999;
		$so_number = qid();
		...
		$query3 .= "'".$I['purchase_item_id']."', 'purchase_item_id', NULL, NULL, 14, 2); ";
		$result3 = qdb($query3) OR die(qe().'<BR>'.$query3);
		$sales_item_id = qid();
echo $query3.'<BR>';
...
echo $query3.'<BR>';
		...
		$query3 = "UPDATE inventory SET returns_item_id = NULL, sales_item_id = '".$sales_item_id."' WHERE id = '".$inventoryid."'; ";
		$result3 = qdb($query3) OR die(qe().'<BR>'.$query3);
echo $query3.'<BR>';
...
echo $query3.'<BR>';
			...
			$query3 .= "VALUES ('".$pcid."', '".$I['purchase_item_id']."', '1', '".$price."'); ";
			$result3 = qdb($query3) OR die(qe().'<BR>'.$query3);
echo $query3.'<BR>';
		}
	}
