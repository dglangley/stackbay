...
$DEBUG = 0;
	...
	while($r = mysqli_fetch_assoc($result)) {
		...
		echo 'Charges: '.getOrderCharges($r['po_number'], $T).' Credits: '.getOrderCredits($r['po_number'], $T) . '<BR>';
		...
		if (getOrderCharges($r['po_number'], $T) <= getOrderCredits($r['po_number'], $T) AND $status == 'Complete') { 
	...
	while($r = mysqli_fetch_assoc($result)) {
		...
		echo 'Charges: '.getOrderCharges($r['so_number'], $T).' Credits: '.getOrderCredits($r['so_number'], $T) . '<BR>';

		// If there are no results of the shipped qty being lower than the qty order than the order is complete
		if (mysqli_num_rows($result2) == 0) { $status = 'Complete'; } else { $status = 'Active'; }

		if (getOrderCharges($r['so_number'], $T) <= getOrderCredits($r['so_number'], $T) AND $status == 'Complete') { 
			echo '<b>Changing Status to Closed</b><BR><BR>' ;
			$status = 'Closed';
		} else { 
			echo '<b>Status Unchanged</b><BR><BR>';
	...
	while($r = mysqli_fetch_assoc($result)) {
		...
		echo 'Charges: '.getOrderCharges($r['ro_number'], $T).' Credits: '.getOrderCredits($r['ro_number'], $T) . '<BR>';

		// If there is no repair line items in the order that does not have a null repair code
		if (mysqli_num_rows($result2) == 0) { $status = 'Complete'; } else { $status = 'Active'; }

		if (getOrderCharges($r['ro_number'], $T) <= getOrderCredits($r['ro_number'], $T) AND $status == 'Complete') { 
			echo '<b>Changing Status to Closed</b><BR><BR>' ;
			$status = 'Closed';
		} else { 
			echo '<b>Status Unchanged</b><BR><BR>';
	...
	while($r = mysqli_fetch_assoc($result)) {
		...
		echo 'Charges: '.getOrderCharges($r['rma_number'], $T).' Credits: '.getOrderCredits($r['rma_number'], $T) . '<BR>';

		// If there is no status line items in the order that does not have a null status code
		if (mysqli_num_rows($result2) == 0) { $status = 'Complete'; } else { $status = 'Active'; }

		if (getOrderCharges($r['rma_number'], $T) <= getOrderCredits($r['rma_number'], $T) AND $status == 'Complete') { 
			echo '<b>Changing Status to Closed</b><BR><BR>' ;
			$status = 'Closed';
		} else { 
			echo '<b>Status Unchanged</b><BR><BR>';
