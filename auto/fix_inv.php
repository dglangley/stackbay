...
$debug = 1;
        ...
        $query = "SELECT * FROM inventory WHERE serial_no = '".$s."' AND qty = 1; ";
...
if (! $debug) { $result3 = qdb($query3) OR die(qe().'<BR>'.$query3); }
echo $query3.'<BR>';

$query3 = "UPDATE inventory_costs_log SET inventoryid = '".$r['id']."' WHERE inventoryid = '".$r2['id']."'; ";
if (! $debug) { $result3 = qdb($query3) OR die(qe().'<BR>'.$query3); }
echo $query3.'<BR>';
...
$query3 = "UPDATE package_contents SET serialid = '".$r['id']."' WHERE serialid = '".$r2['id']."'; ";
if (! $debug) { $result3 = qdb($query3) OR die(qe().'<BR>'.$query3); }
echo $query3.'<BR>';
