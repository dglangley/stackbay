            ...
            if ($service_item_id) {
                ...
                if(mysqli_num_rows($result)) {
                    ...
                    if(mysqli_num_rows($result)) {
                        ...
                        $query = "INSERT INTO parts (part, manfid, systemid, description, classification) VALUES (".fres($part).", ".fres($manfid).", NULL, ".fres($r['description']).", 'component');";
                        qdb($query) OR die(qe() . '<BR>' . $query);
                    ...
                    if($material['po_id']) {
                        ...
                        if(mysqli_num_rows($result)) {
                            ...
                            $query = "INSERT purchase_orders INTO (created, created_by, sales_rep_id, companyid, contactid, assoc_order, remit_to_id, ship_to_id, freight_carrier_id, freight_services_id, freight_account_id, termsid, public_notes, private_notes, status) VALUES ('".res($r5['po_date'])."', '13', '13', '".res(companyMap($r5['vendor_id']))."', NULL, NULL, NULL, NULL, '1', '1', NULL, ".res().", NULL, NULL, 'Active');";
                            qdb($query) OR die(qe() . '<BR>' . $query);
                            $purchase_order = qid();

                            $query = "INSERT purchase_items INTO (partid, po_number, line_number, qty, qty_received, receive_date, ref_1, ref_1_label) VALUES (".res($partid)", ".res($purchase_order).", NULL,".res($material['received_qty']).", ".res($material['received_qty']).", '".$GLOBALS['now']."', ".fres($service_item_id).", 'service_item_id');";
                            qdb($query) OR die(qe() . '<BR>' . $query);
                    ...
                    $query = "INSERT INTO inventory (serial_no, qty, partid, conditionid, status, locationid, userid, date_created, purchase_item_id) VALUES (NULL, ".res($material['received_qty']).", ".res($partid).", '2', 'installed', '149', '13', '".$GLOBALS['now']."', ".res($purchase_item_id).");";
                    qdb($query) OR die(qe() . '<BR>' . $query);
