<?php
	...
	// $query = "DELETE FROM purchase_items WHERE id IN (SELECT purchase_item_id FROM maps_PO_tools); ";
	// $result = qedb($query);
    ...
    foreach($DATA as $order) {
		...
		$userid = mapUser($order['purchase_reo_id']);
        ...
        $query = "INSERT INTO purchase_orders (po_number, created, created_by, sales_rep_id, companyid, ship_to_id, freight_carrier_id, freight_service_id, termsid, status) VALUES (";
            ...
            // Freight ID
            $query .= fres($freight_carrier[$order['freight_carrier_id']]) . ", ";
            $query .= fres($freight_service[$order['freight_carrier_id']]) . ", ";
        ...
        if($po_number) {
            ...
            if($order['tracking_no'] AND $order['tracking_no'] != "No Ship") {
                $query = "INSERT INTO packages (order_number, order_type, package_no, tracking_no, datetime) VALUES (";
                    $query .= fres($po_number) ", ";
                    $query .= "'Purchase', ";
                    $query .= "'1', ";
