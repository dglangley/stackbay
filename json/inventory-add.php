		...
		foreach($item_split as $product) {
			...
			if(is_array ($product[1])) {
				for($i = 0; $i < $product[1].length; $i++) {
					$query  = "INSERT INTO inventory (serial_no, qty, partid, item_condition, status, locationid, last_purchase, last_sale, last_return, repid, date_created, id) VALUES ('". res($product[1][$i]) ."', '". res($product[2]) ."', '". res($product[0]) ."', '". res($product[5][$i]) ."', '". res($product[4][$i]) ."', '". res($product[3][$i]) ."', NULL, NULL, NULL, '1', CAST('". res(date("Y-m-d")) ."' AS DATE), NULL);";
