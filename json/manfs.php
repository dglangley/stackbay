<?php
	...
	if (strlen($q)>1) {
		$query = "SELECT * FROM manfs WHERE name RLIKE '".res($q)."' ";
		...
		while ($r = mysqli_fetch_assoc($result)) {
			...
			$list[$r['id']] = $r['name'];
//			$manfs[] = array('id'=>$r['id'],'text'=>$r['name']);
		}
...
*/
	...
	foreach ($firsts as $r) { $manfs[] = $r; }
	foreach ($seconds as $r) { $manfs[] = $r; }
	foreach ($thirds as $r) { $manfs[] = $r; }

	if ($add_custom) {
		$manfs[] = array('id'=>ucwords($q),'text'=>'Add '.ucwords($q).'...');
	}

	echo json_encode($manfs);//array('results'=>$manfs,'more'=>false));
	exit;
?>
