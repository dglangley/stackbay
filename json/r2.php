<?php
	...
	$query = "SELECT name, companyid cid, ".$T['datetime']." date, ";
...
//		if (count($dates)>=5) { break; }
		...
		if ($pricing) {
			...
			$key = substr($r['date'],0,10).'.'.$r['cid'];//.'.'.$r['partid'];//.'.'.$r['price'];
		}
...
//				} else if ($type=='Purchase' OR $type=='Sale' OR $type=='Service') {
				...
				if ($r['source']) {
					$s = getSearch($r['searchid']);
					$src = setSource($r['source'],$s);

					$res[$key][$k]['sources'][$src['source']] = $s;
					$sources[$src['source']] = $src['ln'];
				}
			}
			continue;
		}
...
//		$dates[substr($r['date'],0,10)] = true;
		...
		if ($type=='Supply' AND ! $r['price'] AND isset($prev_price[$r['cid']]) AND $prev_price[$r['cid']] AND $prev_price[$r['cid']]['date']>$summary_past) {
			$r['price'] = $prev_price[$r['cid']]['price'];
			if ($prev_price[$r['cid']]['date']<$recent_date) { $r['past_price'] = '1'; }
		} else if ($r['price'] AND (! isset($prev_price[$r['cid']]) OR $r['price']<>$prev_price[$r['cid']])) {
//		if (! $prev_price[$r['cid']] AND $r['price'] AND ! $r['past_price']) {
			// set this as a past price only if it's not already a past price
			$prev_price[$r['cid']] = array('date'=>$r['date'],'price'=>$r['price']);
		}
	...
	if ($type=='Purchase') {
		$query = "SELECT 0 cid, requested date, qty, '' price, '0' past_price, po_number order_number, ";
...
//commented here in favor of using below so we can get all results for $range purposes
//		if ($n>=$max_results) { break; }
		...
		if (isset($priced[$date]) AND is_array($priced[$date])) {
			...
			foreach ($priced[$date] as $r) {
				...
				if ($type=='Supply' AND (! strstr($r['name'],'eBay'))) {
					if (! $range['min'] OR $r['price']<$range['min']) {
						$range['min'] = number_format($r['price'],2,'.','');
					}
					if (! $range['max'] OR $r['price']>$range['max']) {
						$range['max'] = number_format($r['price'],2,'.','');
					}
				}
	...
	if ($type=='Purchase') {
		...
		$shelflife = getShelflife($partids,true);
	} else if ($type=='Demand') {
		$PR = getDQ($partids);
	}
...
//	print "<pre>".print_r($final,true)."</pre>"; exit;
