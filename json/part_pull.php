<?php
    ...
    function searchStock($partid, $itemid, $type) {
        ...
        $query = ($itemid != 0 ? "SELECT i.*, 'true' as requested FROM purchase_items pi, inventory i WHERE pi.ref_1_label = '".(strtolower($type) == 'service' ? 'service_item_id' : 'repair_item_id')."' AND pi.ref_1='".res($itemid)."' AND pi.id = i.purchase_item_id AND i.partid = '".res($partid)."' AND i.qty > 0 AND (status = 'shelved' OR status = 'received')
                UNION" : "") . "
