		...
		foreach ($data as $row) {
            ...
            if($row['type'] == 'Purchase') {
                ...
                if(mysqli_num_rows($result) > 0){
                	...
                	$query = "SELECT *, i.price as amount, 'Purchase' as order_type, i.po_number as invoice_no, 'Purchase' as ref_type FROM purchase_items i WHERE i.po_number = '".res($row['order_number'])."';";
	                $result = qdb($query) OR die(qe().' '.$query);
