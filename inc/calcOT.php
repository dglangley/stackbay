...
//			print "<pre>".print_r($shifts,true)."</pre>";
			foreach ($shifts as $r) {
				...
				$otSecs = 0;
				$dtSecs = 0;
//				echo $r['clockin'].' to '.$r['clockout'].' (workdate: '.$r['workdate'].') total shift secs: '.$shiftSecs.' ('.$totalReg.' total reg secs)<BR>';

				// accumulate time toward's 40hr/wk max, but only up to a standard 8hr day per day ($DAY_SECS)
...
//				echo $regSecs.' regular pay secs, '.$cumDaySecs[$r['workdate']].' cumulative day secs, '.$DAY_SECS.' standard pay max secs<BR>';
				// is this shift ($regSecs) PLUS other secs worked on same workday ($cumDaySecs[$r['workdate']]) above a normal workday in secs ($DAY_SECS)?
